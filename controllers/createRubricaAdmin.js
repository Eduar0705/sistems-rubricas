...
router.post('/envioRubrica', (req, res) => {
    ...
    conexion.beginTransaction((err) => {
        ...
        conexion.query(queryRubrica, valuesRubrica, (error, resultRubrica) => {
            if(error) {
            ...
            criteriosParsed.forEach((criterio, indexCriterio) => {
                ...
                conexion.query(queryCriterio, valuesCriterio, (error, resultCriterio) => {
                    if(error) {
                        hayError = true;
                        return conexion.rollback(() => {
                    ...
                    if(criterio.niveles && criterio.niveles.length > 0) {
                        ...
                        criterio.niveles.forEach((nivel, indexNivel) => {
                            ...
                            conexion.query(queryNivel, valuesNivel, (error, resultNivel) => {
                                if(error) {
                                    hayError = true;
                                    return conexion.rollback(() => {
